...
The fit is marked converged when all three maximum absolute component means are
below this threshold.

## Inspect component means

```{r component-diagnostics}
...
- `max_abs_eif_T`: largest failure-hazard EIF component mean across visits
...
Do not rely only on `max_abs_total_eif`. Component means can fail while the
total mean cancels numerically.

## Inspect weights

The diagnostic output includes cumulative treatment-history weight summaries:
...
For baseline randomized treatment with deterministic continuation, weights
should usually be `0` or `1 / g0`. With `g0 = 0.5`, the maximum should be `2`.

For stochastic post-baseline treatment/deviation processes, high maximum weights
or low effective sample size indicate positivity stress. Treat such analyses as
diagnostic until a prespecified stabilization strategy is chosen.
...
3. Run `run_vacttmle_toy_checks()` to verify the installed estimator.
4. Start with `va_ct_gcomp()` to check initial nuisance behavior.
